...
The runbook for the daily picture review. It empties the review queue by looking
at each candidate photograph, records the verdicts and subject boxes, sources
pictures for stories that have none, binds each picture to its story
permanently, and picks a lead that differs from yesterday's. Run it daily, when
the home page images need fixing, or when `verify-homepage` reports frames
without photographs.

You are the reviewer — a person with an image viewer and a text editor, or an
agent with the same. The pipeline finds candidate pictures and does arithmetic;
**the reviewer is the only thing in this system that looks at an image**, and
nothing reaches the home page without a verdict.
...
node scripts/review-queue.js --fetch --out=/tmp/review --limit=25
```
...
```bash
node scripts/review-queue.js --apply=/tmp/review/decisions.json
```
