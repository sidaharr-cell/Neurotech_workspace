...
The architectural record for this repository: what the system is, why it is built
this way, and the rules a change has to respect. Read it before working on
`scripts/`, the ranking pipeline or the image review. For an orientation to the
...
Work on a branch off `main` and merge back to deploy (Vercel builds on push to `main`). Dev server runs on localhost:5173.
...
daily picture review in
[`neurotech-index/docs/home-image-review.md`](neurotech-index/docs/home-image-review.md),
and it is the only thing in the system that looks at an image. See "No script in this repo calls a
vision model" below.
...
`papers.rank_score`, `organizations.rank_score`, `news_feed.relevance_score` (the LLM
judge's 1–10 centrality) and `news_feed.metadata.rankScore`.
...
URL), and meters every model call against `--budget`, checking BEFORE each
...
image steps asked a vision model per candidate per night: is this a photograph, one panel, safe
beside a headline, where is its subject. Those questions are now answered OFFLINE by the
daily reviewer, working from
[`neurotech-index/docs/home-image-review.md`](neurotech-index/docs/home-image-review.md) — and written
...
nothing on the model API — useful when the rail is short and the next cron is a day away.
